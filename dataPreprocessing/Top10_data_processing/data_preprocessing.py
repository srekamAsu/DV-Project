...
def top10_avg_ratings_data(monthly_dic, meta_data, output_json_path, fashion_bad_asin):
    top10_ratings_results = {}
    for i in range(1, 13):
        asin_rating_list = []
        sub_dic = monthly_dic[i]
        for asin in sub_dic:
            if (sub_dic[asin][1] >= 15) and (asin not in fashion_bad_asin):
...
def top10_most_ratings_data(monthly_dic, meta_data, output_json_path, fashion_bad_asin):
    most10_ratings_results = {}
    for i in range(1, 13):
        asin_rating_list = []
        sub_dic = monthly_dic[i]
        for asin in sub_dic:
            if asin not in fashion_bad_asin:
                asin_rating_list.append([asin, sub_dic[asin][0] / float(sub_dic[asin][1]), sub_dic[asin][1]])
...
def fashion_data_processing():
    ...
    top10_avg_ratings_data(ratings_dic, processed_meta_data, 'Fashion/processed_data/top10_avg_ratings.json', fashion_bad_asin)
    top10_most_ratings_data(ratings_dic, processed_meta_data, 'Fashion/processed_data/top10_most_ratings.json', fashion_bad_asin)
